...
func (m *postgresDBRepo) SearchAvailabilityByDatesByRoomID(start, end time.Time, roomID int) (bool, error) {
	...
	query := `
	select count(id) from room_restrictions where
	room_id = $1
...
func (m *postgresDBRepo) SearchAvailabilityForAllRooms(start, end time.Time) ([]models.Room, error) {
	...
	query := `
	select r.id, r.room_name from rooms r where 
	r.id not in (select rr.room_id from room_restrictions rr where $1 <= rr.end_date and $2 >= rr.start_date) order by r.id`
...
// AllReservations returns the slice of all resrevations
...
// AllNewReservations returns the slice of all resrevations
...
// UpdateProcessedForReservation Updates processed for the reservation
